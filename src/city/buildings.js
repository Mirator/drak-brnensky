 ...
 * piano nobile, plainer diminishing uppers and an attic band. One storey
 * equals exactly one texture tile in V, and every wall is snapped to a whole
 * number of bays in U, so windows land symmetrically on each face and
 * different floors genuinely show different windows — which is what gives
 * per-window variety (warm, dark, curtained, the occasional cold TV-blue)
 * without a material per building.
 *
 * Geometry is merged into one mesh per (plaster style x bay kind x lit)
 * bucket, and only the buckets a given city actually uses are ever built.
 */

/** Bay spacing. See the note in the report: the painted bay puts its window
 * at 53% of the tile width, so a 2.9 m module is what keeps the window
 * portrait (never square) at the 3.0-3.4 m upper-storey heights. */
const BAY = 2.9;
...
export function buildHouses(group, collision, plots, rng, { breakables, seed }) {
  const batches = new Batches();

  /* ---- material palette: cached, shared, one instance per variant ---- */
  const litCache = new Map();
  const facade = (style, kind, lit) => {
  ...
  const MAT = {
    ...
    metal: getMaterial('paintedMetal', { seed: 1405, color: '#3f4247' }),
  };
  label(MAT);
  ...
  const emitMass = (mass, plot, isWing) => {
    ...
    for (let i = 0; i < storeys.length; i++) {
      ...
      // string course across the street face at the lower floor lines
      if (corniceRich && i > 0 && i <= 2 && !isWing) {
        const c = Math.cos(rot), s = Math.sin(rot);
        batches.get(MAT.cornice).box(
    ...
    batches.get(MAT.plinth).box(x, 0, z, w + 0.12, 0.55, d + 0.12, rot,
      (face) => [(face === 0 || face === 1 ? d : w) / 1.4, 0.5, 0, 0],
      [skipPlusX, skipMinusX, false, false, true, true]);

    /* ---- main cornice ---- */
    const proj = corniceRich ? 0.72 : 0.44;
    batches.get(MAT.cornice).box(x, eaves - 0.62, z, w + proj, 0.62, d + proj, rot,
    ...
    if (pitched) {
      ...
      if (alongX) {
        batches.get(rmat).gable(x, eaves, z, ow, od, ridgeH, rot, 2.2);
      } else {
        batches.get(rmat).gable(x, eaves, z, od, ow, ridgeH, rot + Math.PI / 2, 2.2);
      }
      roofscape.add(batches, mass, {
        alongX, ridgeH, ow, od, skipPlusX, skipMinusX, isWing,
      });
    } else {
      batches.get(MAT.roofSeam).box(x, eaves, z, w + 0.3, 0.35, d + 0.3, rot,
        (face) => [(face === 0 || face === 1 ? d : w) / 2, 2, 0, 0],
        [false, false, false, false, false, true]);
      // parapet so rooftops still read as places you can be
      const p = batches.get(MAT.cornice);
      ...
      ridgeH = 1.3;
      roofscape.addFlat(batches, mass);
    }

    /* ---- gutters and downpipes at every party wall ---- */
    roofscape.plumbing(batches, mass, { skipPlusX, skipMinusX });
  ...
  const meshes = batches.finish(group);
  const roofMeshes = roofscape.finish(group);
  const shopMeshes = shopfronts.finish(group, collision);
  return {
    meshes: meshes + roofMeshes.meshes + shopMeshes.meshes,
    chimneys: roofMeshes.chimneys,
    dormers: roofMeshes.dormers,
    shops: shopMeshes.shops,
